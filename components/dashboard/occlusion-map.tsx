...
function getPinLabelPlacement(locationId: string, x: number, y: number): { style: CSSProperties; className: string } {
  const anchoredStyle: CSSProperties = {
    left: `${x}%`,
    top: `${y}%`,
  }

  if (locationId === "gate-1-walkway") {
    return {
      style: {
        ...anchoredStyle,
        transform: "translate(0.9rem, -0.4rem)",
      },
      className: "",
    }
  }

  if (locationId === "edsa-sec-walk") {
    return {
      style: {
        ...anchoredStyle,
        transform: "translate(calc(-50% - 0.5rem), 0.85rem)",
      },
      className: "",
    }
  }

  if (locationId === "kostka-walk") {
    return {
      style: {
        ...anchoredStyle,
        transform: "translate(calc(-100% - 1rem), 0.9rem)",
      },
      className: "",
    }
  }

  if (locationId === "gate-3-walkway") {
    return {
      style: {
        ...anchoredStyle,
  ...
  if (nearTop) {
    if (nearLeft) {
      return {
        style: {
          ...anchoredStyle,
          transform: "translate(0.85rem, 0.35rem)",
        },
        className: "",
      }
    }
    ...
    if (nearRight) {
      return {
        style: {
          ...anchoredStyle,
          transform: "translate(calc(-100% - 0.85rem), 0.35rem)",
        },
        className: "",
      }
    }
...
export function OcclusionMap({ hourFilter, onHourFilterChange, data, loading = false }: OcclusionMapProps) {
  ...
  return (
    <div className="rounded-3xl border border-border bg-card p-6 shadow-elevated">
      ...
      <div className="relative h-72 overflow-hidden rounded-2xl border border-white/5 bg-[#0f172a]">
        ...
        {loading ? (
          ...
          <>
            {plottedLocations.map((location) => {
              ...
              const labelPlacement = getPinLabelPlacement(location.id, pos.x, pos.y)
              ...
              return (
                <Fragment key={location.id}>
                  ...
                  <div
                    className={`pointer-events-none absolute z-10 max-w-[9.5rem] rounded-lg border border-white/10 bg-slate-950/90 px-2.5 py-1.5 text-[11px] font-semibold leading-tight text-slate-100 shadow-sm backdrop-blur-sm transition-opacity ${labelPlacement.className} ${isHovered ? "opacity-100" : "opacity-95"}`}
                    style={labelPlacement.style}
                  >
                    <span className="block whitespace-nowrap">{location.name}</span>
                  </div>
      ...
      <div className="mt-4 rounded-2xl border border-border/60 bg-secondary/40 p-3 text-xs text-muted-foreground">
        Neutral markers mean there is either no footage for that location on the selected date or no occlusion-class detections available for the chosen hour.
      </div>
